...
class sensor_cluster():
    ...
    def get_observation(self, rates):
        ...
        #max_clique_throughputs = {max([self._throughputs[node] for node in clique]):clique for clique in maximal_clique_cover} # Get max throughput for each clique
        
        clique_reward_dict = {}
        for clique in maximal_clique_cover:
            clique_reward = max([self._throughputs[node] for node in clique]) / max_throughput
            for node in clique:
                clique_reward_dict[node] = clique_reward 
        
        clique_reward = [np.log(min(1, max(0.1, clique_reward_dict[node]))) for node in range(self._num_sensors)]

        #change_in_throughputs = [(self._throughputs[node] - self._previous_throughputs[node]) / self._throughputs[node] for node in range(self._num_sensors)]
        change_in_total_throughput = total_throughput - self._total_throughput
        self._total_throughput = total_throughput 
        
        throughput_reward = np.log(min(1, max(0.1, change_in_total_throughput + 0.5))) 
        throughput_reward = [throughput_reward for _ in range(self._num_sensors)]
        ...
        return (similarity, self._throughputs, throughput_reward, clique_reward, rates)
